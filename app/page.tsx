...
export default function Home() {
  ...
  return <>
    ...
    <main>
      <section className="hero"><div className="container hero-grid"><div><div className="eyebrow">ONLINE SHOPPING IN RWANDA</div><h1>Shop what you need.<br />We deliver.</h1><p>Discover goods, choose what you need, place your order online, and receive your order details.</p><div className="actions"><a className="btn btn-primary" href="#products">Start shopping</a><a className="btn btn-secondary" href="#how">How it works</a></div></div><div className="hero-card"><div className="badge">Live store</div><h2>Your order, from click to delivery</h2><p className="muted">Products → Cart → Customer details → Order confirmation</p><div className="amount">{cartCount} item{cartCount === 1 ? '' : 's'} in cart</div><div className="muted">The website and future mobile app can share the same backend.</div></div></div></section>
      <section id="products" className="section"><div className="container"><h2>Shop products</h2><p className="muted">Choose a product and add it to your cart.</p>{loading ? <p>Loading products...</p> : products.length === 0 ? <div className="note">No products are available yet. Add products from the admin area.</div> : <div className="products">{products.map(product => <article className="product" key={product.id}><div className="product-image">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} /> : '🛍️'}</div><div className="product-body"><div className="badge">{product.category?.name ?? 'Featured'}</div><h3>{product.name}</h3><div className="price">{money(product.priceRwf)}</div><div className="muted">{product.stock} in stock</div><button className="btn btn-primary" style={{ marginTop: 14, width: '100%' }} onClick={() => addToCart(product)} disabled={product.stock < 1}>Add to cart</button></div></article>)}</div>}{message && <div className="note" style={{ marginTop: 18 }}>{message}</div>}</div></section>
