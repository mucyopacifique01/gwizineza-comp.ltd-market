...
export default function Home() {
  ...
  return <>
    <header className="nav"><div className="container nav-inner"><div className="logo">Gwizineza<span> Market</span></div><nav className="nav-links"><a href="/shop">Shop</a><a href="#products">Products</a><a href="#how">How it works</a><a href="#location">Location</a><a href="#checkout">Checkout</a><a href="/auth">Sign in / Sign up</a></nav><button className="btn btn-primary" onClick={() => setCheckoutOpen(true)}>Cart ({cartCount})</button></div></header>
    <main>
      <section className="hero"><div className="container hero-grid"><div><div className="eyebrow">ONLINE SHOPPING IN RWANDA</div><h1>Shop what you need.<br />We deliver.</h1><p>Discover goods, choose what you need, place your order online, and receive your order details.</p><div className="actions"><a className="btn btn-primary" href="/shop">Start shopping</a><a className="btn btn-secondary" href="#how">How it works</a></div></div><div className="hero-card"><div className="badge">Live store</div><h2>Your order, from click to delivery</h2><p className="muted">Products → Cart → Customer details → Order confirmation</p><div className="amount">{cartCount} item{cartCount === 1 ? '' : 's'} in cart</div><div className="muted">The website and future mobile app can share the same backend.</div></div></div></section>
      <section id="products" className="section"><div className="container"><h2>Shop products</h2><p className="muted">Choose a product and add it to your cart.</p>{loading ? <p>Loading products...</p> : products.length === 0 ? <div className="note">No products are available here yet. <a href="/shop">Browse the full shop</a> or add products from the admin area.</div> : <div className="products">{products.map(product => <article className="product" key={product.id}><div className="product-image">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} /> : '🛍️'}</div><div className="product-body"><div className="badge">{product.category?.name ?? 'Featured'}</div><h3>{product.name}</h3><div className="price">{money(product.priceRwf)}</div><div className="muted">{product.stock} in stock</div><button className="btn btn-primary" style={{ marginTop: 14, width: '100%' }} onClick={() => addToCart(product)} disabled={product.stock < 1}>Add to cart</button></div></article>)}</div>}{message && <div className="note" style={{ marginTop: 18 }}>{message}</div>}</div></section>
      <section id="location" className="section"><div className="container"><div className="checkout"><div><div className="eyebrow">Our location</div><h2>Gwizineza Market — Kabarondo, Rwanda</h2><p className="muted">Our business location is in Kabarondo, Kayonza District, Eastern Province, Rwanda.</p><p className="muted">Approximate map coordinates: <strong>-2.0127, 30.5585</strong>.</p><a className="btn btn-secondary" href="https://www.openstreetmap.org/?mlat=-2.0127&mlon=30.5585#map=15/-2.0127/30.5585" target="_blank" rel="noreferrer">Open map</a></div><div style={{ overflow: 'hidden', borderRadius: 16, minHeight: 320, border: '1px solid #ddd' }}><iframe title="Gwizineza Market location in Kabarondo, Rwanda" src="https://www.openstreetmap.org/export/embed.html?bbox=30.535%2C-2.030%2C30.582%2C-1.995&layer=mapnik&marker=-2.0127%2C30.5585" style={{ width: '100%', height: 320, border: 0 }} loading="lazy" /></div></div></div></section>
