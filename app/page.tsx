...
export default function Home() {
  ...
  const [form, setForm] = useState({ customerName: '', phone: '', tin: '', deliveryAddress: '' });
  ...
  return (
    <>
      ...
      <main>
        <section className="hero">
          <div className="container hero-grid">
            <div>
              ...
              <p>Discover goods, order online, and receive your order details. Online payment will be added later.</p>
        ...
        <section id="how" className="section"><div className="container"><div className="checkout"><div><div className="eyebrow">Simple ordering</div><h2>Order now, payment later</h2><p>For now, customers can provide their name, phone number, optional TIN, and delivery address. The server creates the order and reserves stock. MTN payment, payment verification, EBM, and WhatsApp receipt delivery will be added later.</p></div><div id="checkout" className="form"><label>Customer name</label><input value={form.customerName} onChange={(e) => setForm({ ...form, customerName: e.target.value })} placeholder="Your name" /><label>Customer phone / WhatsApp</label><input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="07XX XXX XXX" /><label>TIN (optional)</label><input value={form.tin} onChange={(e) => setForm({ ...form, tin: e.target.value })} placeholder="Enter TIN" /><label>Delivery address</label><input value={form.deliveryAddress} onChange={(e) => setForm({ ...form, deliveryAddress: e.target.value })} placeholder="Kabarondo, district, sector..." /><button className="btn btn-primary" onClick={() => setCheckoutOpen(true)} disabled={cartCount === 0}>Review cart & place order</button><div className="note">No payment is requested at this stage. You can test the shopping and ordering flow first.</div></div></div></div></section>

        {order && <section className="section"><div className="container"><div className="checkout"><div><div className="eyebrow">Order created</div><h2>Order {order.orderNumber}</h2><p>Your order has been created successfully. Payment will be added later.</p></div><div><div className="price">{money(order.totalRwf)}</div><div className="muted">Status: {order.status}</div></div></div></div></section>}
      </main>
